...
def extract_data_from_pdf(pdf_path, output_xls_path):
    ...
    sheet.pictures.add(image_path, name='MyImage', update=True, left=sheet.range(cell_location).left, top=sheet.range(cell_location).top)

    wb.save(output_xls_path)
    wb.close()

def main():
    parser = argparse.ArgumentParser(description="Extract data from PDF and save to Excel.")
    parser.add_argument("pdf_path", help="Path to the PDF file to extract data from.")
    parser.add_argument("output_xls_path", help="Path for the output Excel file (.xls).")
